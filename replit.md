...
- **App name rule:** the app must always have the most literal, utilitarian name possible (e.g. "Data Analytics", "Ethics", "Formal Logic", "Basic AI"). Never give it a goofy, funny, or cute name. Use this exact name consistently in the README, the video, and everywhere else relevant.
- **YouTube description title rule:** the title line in any YouTube description must ALWAYS be exactly `X — AI-Powered Course` (e.g. `Basic Developmental Psychology — AI-Powered Course`). No descriptors, taglines, alternates, or extra words after "AI-Powered Course".
